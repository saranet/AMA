...
class ProfilePageController extends GetxController {
  var userName = TextEditingController(
      text: AppStorageController.to.currentUser?.username);
      ...
      "Sunday",
        ...
        .map(
          (e) => WorkingDaysModel(
            label: e,
            code: e.toUpperCase(),
            isSelected: AppStorageController.to.currentUser?.wrokingDays
  ...
  void onReady() {
    super.onReady();
    var startTimeTemp =
        AppStorageController.to.currentUser?.inTime ?? "00:00:00";
    var endTimeTemp =
        AppStorageController.to.currentUser?.outTime ?? "00:00:00";
    startTime.value = TimeOfDay(
        hour: int.tryParse(startTimeTemp.split(":")[0]) ?? 0,
        minute: int.tryParse(startTimeTemp.split(":")[1]) ?? 0);
    endTime.value = TimeOfDay(
        hour: int.tryParse(endTimeTemp.split(":")[0]) ?? 0,
        minute: int.tryParse(endTimeTemp.split(":")[1]) ?? 0);
  }
    ...
    if (AppStorageController.to.currentUser?.roleType !=
      ...
      if (resp is Map<String, dynamic>) {
        if (resp['status']) {
          ...
          AppStorageController.to.currentUser?.wrokingDays =
              (resp['data']['workingDays'] as List<dynamic>).cast<String>();
          AppStorageController.to.currentUser?.outTime =
              resp['data']['outTime'];
